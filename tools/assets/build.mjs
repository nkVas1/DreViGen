...
for(const variant of ['light','dark','print']){
  ...
  // The web delivery format sits beside the master rather than in generated/, because the
  // interface loads it on first paint and a clean clone has to have it without running this
  // script. Lossy, deliberately: the PNG is 189 KB and lossless WebP barely improves on it,
  // because dense noise is what compression cannot do. At the 3.5-5.5% opacity this tile is
  // ever composited at, a quality-80 artefact is far below anything an eye can resolve.
  const webp=rel.replace(/\.png$/,'.webp');
  await sharp(path.join(assets,rel)).webp({quality:80,effort:6}).toFile(path.join(assets,webp));
  record('03',`grain-${variant}`,{light:'Бумага · дневной свет',dark:'Бумага · лампа',print:'Бумага · печать'}[variant],rel,{method:'procedural',width:512,height:512,seamless:true,formats:{png:rel,webp}});
}
...
// The interface imports geometry, not files. Eighty `<img>` elements cannot take
// `currentColor`, an external sprite ties the icons to an origin the Tauri build does not
// share with the web build, and inlining raw markup would mean trusting a string. Structured
// primitives render as real elements, tree-shake, and type-check.
const primitives=body=>body.split('|').map((part,index)=>{
  if(index===0)return part?{kind:'path',d:part}:null;
  const [kind,x,y,r]=part.trim().split(' ');
  if(kind!=='c')throw new Error(`Unknown glyph primitive: ${part}`);
  return {kind:'circle',cx:Number(x),cy:Number(y),r:Number(r)};
}).filter(Boolean);
const iconSource=[
  ...
  '}',
  '',
  'export const icons = '+JSON.stringify(Object.fromEntries(iconManifest.map(i=>[i.name,{label:i.label,category:i.category,shapes:primitives(groups.flatMap(([,,list])=>list).find(([n])=>n===i.name)[2])}])),null,2)+' as const;',
  '',
...
await write(path.join(root,'packages/ui/src/icons.generated.ts'),iconSource);
